...
var config = {
  apiOrigins: {
    production: 'https://murmuring-sierra-58376.herokuapp.com'
  }
};
...
var onSignIn = function onSignIn(event) {
  ...
  // console.log(data)
  event.preventDefault();
  api.signIn(data).then(ui.signInSuccess).then(api.getNotes).then(checkGet).catch(ui.signInFailure);
};
...
var onCreateNote = function onCreateNote(event) {
  ...
  //   }
  // }
  api.createNote(data).then(ui.createNoteSuccess).catch(ui.createNoteFailure);
};
var onGetNotes = function onGetNotes(event) {
  event.preventDefault();
  // const data = getFormFields(this)
  api.getNotes().then(ui.getNotesSuccess).catch(ui.getNoteFailure);
};

var onCollapse = function onCollapse(event) {
  event.preventDefault();
  ui.collapseTable();
};
var checkGet = function checkGet(data) {
...
var removeNotes = function removeNotes(noteId) {
  return $.ajax({
...
//   })
// }
var editNotes = function editNotes(data, noteId) {
  return $.ajax({
    url: config.apiOrigin + '/notes/' + noteId,
    method: 'PATCH',
    headers: {
...
var store = __webpack_require__(7);
var api = __webpack_require__(6);
var showNotesTemplate = __webpack_require__(16);
var events = __webpack_require__(5);
...
var getNotesSuccess = function getNotesSuccess(data) {
  // console.log('data notes is ' + data)
  if (data.notes.length !== 0) {
    // $('#message').text('Got em!').fadeIn().delay(8000).fadeOut()
    var showNotesHtml = showNotesTemplate({ notes: data.notes });
    $('.notes-table').show();
    $('#collapseNotesButton').show();
    $('#getNotesButton').show();
    $('.note-listing').show();
...
var onRemoveNote = function onRemoveNote(e) {
  var noteId = $(this).parent().parent().attr('data-id');
  console.log(noteId);
  // console.log(noteId)
  $(this).parent().parent().remove();
  api.removeNotes(noteId).then(removeNoteSuccess).catch(failure);
};

var removeNoteSuccess = function removeNoteSuccess(event) {
  $('#message').text('Deleted note!').fadeIn().delay(4000).fadeOut();
  api.getNotes().then(getNotesSuccess).catch(removeNoteFailure);
};

var onEditNote = function onEditNote(event) {
  // const data = getFormFields(this)
  event.preventDefault();
  $('#edit-info').show();
  var noteId = $(this).parent().parent().attr('data-id');
  var comment = $(this).parent().siblings()[0];
  var time = $(this).parent().siblings()[1];
  ...
  $('.edit-cancel').on('click', function (event) {
    ...
    $('.edit-cancel').hide();
    // $('#collapseNotesButton').hide()
    api.getNotes().then(getNotesSuccess);
  });
...
var collapseTable = function collapseTable() {
  clearTable();
  $('#collapseNotesButton').hide();
  $('#edit-info').hide();
};
...
var onNoteEdit = function onNoteEdit(noteId, comment, time) {
  $('#edit-info').hide();
  var newComment = $(comment).html();
  var newTime = $(time).html();
  var data = {
    note: {
      comment: newComment,
      time: newTime
  ...
  api.editNotes(data, noteId).then(editNoteSuccess).catch(failure);
};
var createNoteSuccess = function createNoteSuccess(data) {
  // console.log(data)
...
var editNoteSuccess = function editNoteSuccess(data) {
  // console.log(data)
  $('#message').text('You edited a note!').fadeIn().delay(4000).fadeOut();
  // store.notes = data.notes
  // clearTable()
  api.getNotes().then(getNotesSuccess).catch(failure);
};
// //////////////////
...
var createNoteFailure = function createNoteFailure(data) {
  // console.log(data)
  $('#message').text('Please fill out comment!').fadeIn().delay(4000).fadeOut();
  $('#note').trigger('reset');
};
var getNoteFailure = function getNoteFailure(data) {
  // console.log(data)
  $('#message').text('Got nothin, make a note!').fadeIn().delay(4000).fadeOut();
};
var removeNoteFailure = function removeNoteFailure(data) {
  // console.log(data)
...
module.exports = {
  ...
  getNotesSuccess: getNotesSuccess,
  getNoteFailure: getNoteFailure,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n  <tr data-id=\""
    ...
    + "</td>\n    <td>\n    <button class=\"remove\"> Delete Note</button>\n    <button class=\"edit-note\">Edit Note</button>\n  </td>\n    <!-- <form class=\"form-inline edit-note\">\n        <input type=\"text\" name=\"note[comment]\" value="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.comment : stack1), depth0))
    + "> <br>\n        <input type=\"datetime\" name=\"note[time]\" value="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.time : stack1), depth0))
    + "> <br>\n        <li><button class='edit-note'>Edit Note!</button></li>\n      </form> -->\n  </tr>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.notes : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
